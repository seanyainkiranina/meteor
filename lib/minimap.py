...
class MiniMap:
    ...
    def draw(self, plane, meteors, aliens, people):
        """ draw map """
        ...
        pygame.draw.rect(self._surface, self._border,
                         (0, 0, self._width, self._height), 2)
        ...
        for a in aliens:
            if camera_left <= a._world_x <= camera_right:
                mx, my = world_to_map(a._world_x, a._y)
        ...
        self._screen.blit(self._surface,
                          (self._screen.get_width() - self._width - 10, 10))
